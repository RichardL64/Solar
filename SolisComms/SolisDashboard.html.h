  ...
  <script>
    ...
    function callServer(freq, url) {
      ...
      fetch('http://'
        ...
        .then(json => {                                     // body
          if(json[BATTCD_R] == 0) json[BATTERY_R] *= -1     // Make battery signed - 0=chg, 1=dischg
          delete json[BATTCD_R]                             // do this here to avoid issues with animation frames on the 0/1 value

          json[HOUSE_R] = json[SOLAR_R] - json[GRID_R] + json[BATTERY_R]
    ...
    function updateDashboard() {
      ...
      setValue("batterySOCG", 0,      100, dispJson[BATTSOC_R], dispJson[BATTSOC_R].toFixed(0) + "%");
